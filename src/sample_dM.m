function dM = sample_dM(Y,M,A,dM,X,Sig,Psi,nu_dM)
%Sample the variability matrix dM s.t. 0 <= M+dM (see (52)-(54)).
%
%-------------------------------------------------------------------------%
%%
...
for l = 1:L
    for r = 1:R
        ...
        dM(l,r,1) = rtnorm(-M(l,r),Inf,mu_lr,S(r,l,1)); % if projection onto lower dimensional subspace (PCA...)
        id(r) = true;
    end
end
...
for t = 2:T-1
    for l = 1:L
        for r = 1:R
            ...
            dM(l,r,t) = rtnorm(-M(l,r),Inf,mu_lr,S(r,l,t)); % if projection onto lower dimensional subspace (PCA...)
            id(r) = true;
        end
    end
end
