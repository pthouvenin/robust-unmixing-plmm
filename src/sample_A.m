function A = sample_A(Y,M,A,dM,Z,X,Eps,Sig)
...
%-------------------------------------------------------------------------%
%%
% Version without the sum-to-one constraint
...
for t = 1:T
    for n = 1:N
        if (Z(n,t) == 0)
            ...
            % Sample A (Gaussian truncated to the intersection of the l1 ball (0,1) and of the positive orthant)
            A(:,n,t) = gibbs_tg_splx(Lambda0(:,:,t)\mu0(:,n,t),inv(Lambda0(:,:,t)),A(:,n,t)); % somme <= 1
            % otherwise, sample a_{n,t} \in [0,1] -> gaussian distribution truncated to [0,1]
            % A(:,n,t) = gibbs_tg(Lambda0(:,:,t)\mu0(:,n,t),inv(Lambda0(:,:,t)),A(:,n,t),zeros(R,1),ones(R,1));
        end
    end    
end
